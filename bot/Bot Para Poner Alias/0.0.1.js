...
(function() {
    ...
    let userMessageCounts = new Map(); // Nuevo: Contador de mensajes por usuario
    ...
    function showNotification(message, type = 'info', duration = 3000) {
        const colors = {
            ...
            top: 'linear-gradient(135deg, #FF9800, #FF5722)'
        };
    ...
    // NUEVA FUNCIÓN: Actualizar contador de mensajes por usuario
    function updateUserMessageCount(username) {
        if (!username || username === 'Usuario' || username === 'Tú' || username === 'You') {
            return;
        }
    ...
    function scanMessages() {
        ...
        for (let i = startIndex; i < messageElements.length; i++) {
            ...
            if (!scannedMessages.has(messageData.id)) {
                ...
                updateUserMessageCount(messageData.username);
                ...
                if (messageData.isCommand && !messageData.isYou) {
                    ...
                    } else if (content === '/top msg' || content === '/top' || content === '/topmsg') {
                        const commandId = `topmsg_${messageData.username}_${Date.now()}`;
                        const now = Date.now();
                        const lastProcessed = processedCommands.get(commandId) || 0;
        ...
        if (newCommands.length > 0) {
            ...
            newCommands.forEach((cmd, index) => {
                setTimeout(() => {
                    ...
                    } else if (cmd.type === 'topmsg') {
                        processTopMsgCommand(cmd.data);
                    }
                }, index * 1500);
            });
        }
        ...
        if (newMessages > 0) {
            log(`📊 ${newMessages} mensaje(s) nuevo(s)`, {
                total: scannedMessages.size, 
                roles: userRoles.size,
                users: userMessageCounts.size
            }, 'info');
        }
    ...
    // NUEVA FUNCIÓN: Procesar comando /top msg
    function processTopMsgCommand(messageData) {
        if (userMessageCounts.size === 0) {
            sendToChat(`📭 **No hay datos de mensajes aún**\n\n💡 El sistema necesita escanear más mensajes para generar el top.\n⏳ Sigue chateando y vuelve a intentarlo.`);
            return;
        }
        
        log(`🏆 Procesando /top msg`, {users: userMessageCounts.size}, 'top');
        
        // Convertir Map a array y ordenar por cantidad de mensajes (descendente)
        const sortedUsers = Array.from(userMessageCounts.entries())
            .sort((a, b) => b[1] - a[1]) // Ordenar por cantidad de mensajes (mayor a menor)
            .slice(0, 10); // Tomar solo los top 10
        
        // Calcular total de mensajes escaneados
        const totalMessages = Array.from(userMessageCounts.values())
            .reduce((sum, count) => sum + count, 0);
        
        let response = `🏆 **TOP 10 ACTIVOS DEL CHAT** 🏆\n\n`;
        response += `📊 **Mensajes escaneados:** ${totalMessages}\n`;
        response += `👥 **Usúarios únicos:** ${userMessageCounts.size}\n\n`;
        response += `📈 **RANKING DE MENSAJES** 📈\n\n`;
        
        sortedUsers.forEach(([username, count], index) => {
            // Asignar medallas o números
            let rankPrefix;
            switch(index) {
                case 0: rankPrefix = '🥇 **1er LUGAR**'; break;
                case 1: rankPrefix = '🥈 **2do LUGAR**'; break;
                case 2: rankPrefix = '🥉 **3er LUGAR**'; break;
                default: rankPrefix = `**${index + 1}.**`; break;
            }
            ...
            const barLength = 20;
            const filledBars = Math.round((count / sortedUsers[0][1]) * barLength);
            const progressBar = '█'.repeat(filledBars) + '░'.repeat(barLength - filledBars);
            
            response += `${rankPrefix} **${username}**\n`;
            response += `   📨 **${count}** mensajes (${percentage}%)\n`;
            response += `   ${progressBar}\n\n`;
        });
        
        // Añadir estadísticas adicionales
        const mostActiveUser = sortedUsers[0];
        const averagePerUser = (totalMessages / userMessageCounts.size).toFixed(1);
        
        response += `📊 **ESTADÍSTICAS** 📊\n`;
        response += `👑 **Más activo:** ${mostActiveUser[0]} (${mostActiveUser[1]} mensajes)\n`;
        response += `📊 **Promedio por usuario:** ${averagePerUser} mensajes\n`;
        response += `⏱️ **Escaneo iniciado:** ${getTimeAgo(window.scanStartTime || Date.now())}\n\n`;
        
        response += `💡 **Comandos disponibles:**\n`;
        response += `• \`/top msg\` - Ver este ranking\n`;
        response += `• \`/rol @usuario mensaje\` - Asignar rol\n`;
        response += `• \`/roltop\` - Ver roles asignados\n\n`;
        
        response += `✨ *¡Sigue participando para subir en el ranking!* ✨`;
        
        sendToChat(response);
        showNotification(`🏆 Top 10 de mensajes generado`, 'top', 3000);
    }
    ...
    function startScanner() {
        ...
        window.scanStartTime = Date.now(); // Guardar tiempo de inicio para estadísticas
    ...
    function createControlPanel() {
        ...
        panel.innerHTML = `
            <div style="text-align: center; margin-bottom: 15px;">
                ...
                <div style="font-size: 11px; color: rgba(255,255,255,0.7); margin-top: 5px;">
                    Envío garantizado + Top Mensajes
                </div>
            </div>
            
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin: 15px 0; text-align: center;">
                ...
                <div style="background: rgba(255,255,255,0.08); padding: 8px; border-radius: 6px;">
                    <div style="font-size: 10px;">👥 Users</div>
                    <div id="stat-users" style="font-size: 16px; font-weight: bold; color: #FF9800;">0</div>
                </div>
            </div>
        ...
        setInterval(() => {
            ...
            panel.querySelector('#stat-users').textContent = userMessageCounts.size;
        }, 1000);
    ...
    function initialize() {
        ...
        console.log('🏆 CON COMANDO /top msg');
        console.log('='.repeat(70));
        ...
        console.log('• Top 10 de usuarios más activos');
        console.log('='.repeat(70));
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                sendToChat(`🎭 **¡SISTEMA DE ROLES ACTIVADO!** 🎭\n\n` +
                          ...
                          `🏆 **NUEVO COMANDO: /top msg** 🏆\n` +
                          `Muestra el ranking de usuarios más activos\n\n` +
                          `📌 **Comandos disponibles:**\n` +
                          `• \`/rol @usuario mensaje_del_rol\`\n` +
                          `• \`/roltop\` - Ver todos los roles\n` +
                          `• \`/top msg\` - Top 10 usuarios más activos\n\n` +
                          `💡 **Ejemplo rápido:**\n` +
                          `\`/rol @${Object.keys(userRoles)[0] || "Usuario"} el más divertido\`\n\n` +
                          `🎮 *¡Diviértete con los roles y el ranking!*`);
            }, 3000);
    ...
    window.RolesUltra = {
        ...
        getStats: () => ({
            ...
            roles: userRoles.size,
            users: userMessageCounts.size,
            topUsers: Array.from(userMessageCounts.entries())
                .sort((a, b) => b[1] - a[1])
                .slice(0, 5),
            isRunning: scanInterval !== null
        }),
        getTop: () => {
            return Array.from(userMessageCounts.entries())
                .sort((a, b) => b[1] - a[1])
                .slice(0, 10);
        }
    };
